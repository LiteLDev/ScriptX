...
namespace script::v8_backend {
 ...
 public:
  ...
  void PostTaskOnWorkerThreadImpl(v8::TaskPriority priority,
    ...
    return defaultPlatform_->CallOnWorkerThread(std::move(task), location);
  }
  ...
  void PostDelayedTaskOnWorkerThreadImpl(v8::TaskPriority priority, std::unique_ptr<v8::Task> task,
    ...
    return defaultPlatform_->CallDelayedOnWorkerThread(std::move(task), delay_in_seconds, location);
  }
...
#define V8_VERSION_AT_LEAST(major, minor) \
  (V8_MAJOR_VERSION > (major) || (V8_MAJOR_VERSION == (major) && V8_MINOR_VERSION >= (minor)))
...
#if (defined(BUILDING_NODE_EXTENSION) ? V8_VERSION_AT_LEAST(8, 6) : V8_VERSION_AT_LEAST(8, 4)) && !V8_VERSION_AT_LEAST(12, 4)
        ...
        std::unique_ptr<v8::JobHandle> CreateJobImpl(
                ...
                const v8::SourceLocation &location) override {
            return defaultPlatform_->PostJob(priority, std::move(job_task), location);
        }
